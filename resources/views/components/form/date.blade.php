...
<div @class(['w-full', $wrapperClass])>
    ...
    >
        ...
        <input
            {{ $attributes->merge([
                'type' => 'date',
                'class' => 'h-full w-full border-0 bg-transparent px-3 text-sm text-zinc-900 outline-none ring-0 focus:ring-0 focus:outline-none disabled:cursor-not-allowed',
            ]) }}
            @disabled($disabled)
        />
    </div>
