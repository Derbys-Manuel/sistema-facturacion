...
class DistrictSeeder extends Seeder
{
    public function run(): void
    {
        ...
        /** @var array<string, string> $provinceIdByCode */
        $provinceIdByCode = DB::table('provinces')->pluck('id', 'code')->all();

        $rows = collect($ubigeo_peru_districts)->map(fn (array $district): array => [
            'id' => (string) Str::uuid(),
            'code' => $district['id'],
            'description' => trim($this->cleanText($district['name'])),
            'province_id' => $provinceIdByCode[$district['province_id']] ?? null,
            'is_active' => true,
            'created_at' => $now,
            'updated_at' => $now,
        ])->all();

        DB::table('districts')->upsert($rows, ['code'], ['description', 'province_id', 'is_active', 'updated_at']);
    }
