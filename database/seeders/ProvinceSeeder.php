...
class ProvinceSeeder extends Seeder
{
    public function run(): void
    {
        ...
        /** @var array<string, string> $departmentIdByCode */
        $departmentIdByCode = DB::table('departments')->pluck('id', 'code')->all();

        $rows = collect($ubigeo_peru_provinces)->map(fn (array $province): array => [
            'id' => (string) Str::uuid(),
            'code' => $province['id'],
            'description' => trim($this->cleanText($province['name'])),
            'department_id' => $departmentIdByCode[$province['department_id']] ?? null,
            'is_active' => true,
            'created_at' => $now,
            'updated_at' => $now,
        ])->all();

        DB::table('provinces')->upsert($rows, ['code'], ['description', 'department_id', 'is_active', 'updated_at']);
    }
